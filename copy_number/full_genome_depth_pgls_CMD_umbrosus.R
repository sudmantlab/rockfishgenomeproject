...
files <- list.files("../../data/gene_copies/umbrosus_genome/full_genome_scan", paste0(scaf,".depth.gz"))
...
for (i in files){
  ...
  data <- read_tsv(paste0("../../data/gene_copies/umbrosus_genome/full_genome_scan/",i),
...
region_depth %>%
  ...
  filter(species  != "Sebastes_umbrosus") %>%
...
#for (i in tail(unique(region_depth_scaled$start),100)){
...
write_tsv(pgls_results, 
          paste0("../../data/gene_copies/umbrosus_genome/full_genome_scan_results/pgls_",scaf,".txt.gz"))
